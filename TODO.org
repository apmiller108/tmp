...
* TODO show meta data about the assistant response (model, present, temp, token count) [6/10]
  ...
  - [ ] Can we remove the code that creates the conversation too? And ditch that
    form object?
  - [ ] Remove the code that updates the conversation from the memo feature.
  - [ ] Replace memos stream with main
