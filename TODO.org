...
* TODO add conversation title [3/4]
  - [X] Show the title at the top in a fixed container.
  - [ ] Show form on click of edit icon (hide field) and put focus on input
  - [ ] Hide form and show field when input loses focus
  - [ ] Extract partial
  - [ ] On update, render turbo stream partial
  - [X] add title to conversaions table
  - [X] set title default based on first message
* TODO show meta data about the assistant response (model, present, temp, token count) [/]
  - [ ] add response jsonb field to generate_text_requests
