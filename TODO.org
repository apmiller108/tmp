...
* TODO show meta data about the assistant response (model, present, temp, token count) [4/8]
  ...
  - [ ] Remove code that updates the conversation from the conversations view. Drop the form object. Do we
    still need a separate controller for the memo conversations?
  - [ ] Remove the code that updates the conversation from the memo feature.
  - [ ] Drop the exchange column and remove the exchange attr
