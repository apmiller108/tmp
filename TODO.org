...
* TODO show meta data about the assistant response (model, present, temp, token count) [1/8]
  - [X] add response jsonb field to generate_text_requests
  - [ ] update generate_text_request record with the full response.
  ...
  - [ ] when displaying the turns, do so from the generate text requests
    ...
    - Conversations has an turns method that maps each message or response to a
      ...
      generate_text_request prompt and response. Use the helper method for the
      prompt. Use helper method on the response obj.content. Wrap the json blob
      in a InovkeModelResponse obj.
