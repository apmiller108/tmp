...
* TODO add image generation to conversations [7/17]
  ...
  - [ ] Link the generate image request to the conversation.
  - [ ] Wrap a database view in a model associate to a conversation (ConversationTurn)
