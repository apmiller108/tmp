...
* TODO Conversation Stats
  Images, other media, tokens...
...
* TODO Image sketch
* TODO List view of generated images
  or list of all conversation files?
...
* TODO Wire up search
  ...
  - [ ] Create query object
    #+begin_src ruby
      # app/queries/conversation_search.rb
        class ConversationSearch
          ...
          def call
            apply_memo_filter
            apply_vector_search if search_term.present?
            relation
          end
          ...
          def apply_memo_filter
            return unless params.dig(:q, :memo_id)

            @relation = relation.where(memo_id: params.dig(:q, :memo_id))
          end

          def apply_vector_search
            return unless search_term.present?

            query_vector = Embeddings::Voyage.create_embedding(
              text: search_term,
              input_type: :query
            )

            # Using the neighbors gem for nearest neighbor search
            # Adjust the distance threshold and limit as needed
            @relation = relation.nearest_neighbors(:embeddings, query_vector, distance_threshold: 0.8)
          end

          def search_term
            params.dig(:q)&.is_a?(String) ? params[:q] : params.dig(:q, :term)
          end
        end
    #+end_src

    #+begin_src haml
     = form.search_field :q, name: 'q[term]', class: 'form-control', placeholder: 'Search conversations...', 'aria-label' => 'Search', data: { 'search-modal-target' => 'input', action: 'keydown.enter->search-modal#search:prevent' } 
    #+end_src
  - [ ] Add title to blobification
  - [ ] Create search service to create embeddings for a search query
  - [ ] Use search query embedding to perform query
