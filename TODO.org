...
* TODO redirect to edit view on first generate text request response with a disabled form [0/8]
  - [ ] add accepts_nested_attributes_for generate_text_request to conversation
  - [ ] Change the prompt form component to the conversation form component.
    Most of the fields will be for the generate text request
  - [ ] Submit the form to the conversation post/put endpoints
  - [ ] Redirect to conversation edit on create. Enqueue the GenerateTextJob
  - [ ] Render the form and conversation turn components from the update action.
    Enqueue the GenerateTextJob
  - [ ] Remove rendering the turbo streams from the generate text requests controller
  - [ ] Remove the hack that sets the browser history state
  - [ ] Make sure the title is editable
  - [ ] Make sure generating text still works for memos
* TODO turn meta data
