...
* TODO Remove conversation::turn classes
  - [ ] Use the gtrs in the conversation view. Refactor the
    conversation_turn_component to get the data from gtr. It will render both
    the user and assistant response
  - [ ] Add the concept of pending_response to gtr. Use that to determine when
    to show the spinner
  - [ ] Implement a to_message_turn method on gtr that will return a tuple of
    properly formatted user and assistant hashes that will be serialied for the
    http request
  - [ ] For the exchange loop over the gtrs and call to_message_turn to produce
    the tuple of user, assistant response
  - [ ] Delete the conversaion::turn objects
